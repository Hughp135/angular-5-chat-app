...
voiceChannelSchema.pre('save', async function(next) {
  ...
  if (!this.isNew || !(<IVoiceChannelModel>this).server_id) {
    // Only check newly created server channels
    return next();
  }
  const channel = <IVoiceChannelModel>this;
  const otherChannel = await (<any>this.constructor)
    .findOne({
...
const VoiceChannel: mongoose.Model<IVoiceChannelModel> = mongoose.model<
  IVoiceChannelModel
>('VoiceChannel', voiceChannelSchema);
